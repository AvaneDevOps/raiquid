...
| `InvoiceRef`                                                                              | `shared/domain/status-badges.tsx`                        | Amber monospace id chip, e.g. `RQ-INV-4471`, for the invoice-detail header. Same seal-chip shape; keeps the id's own casing. Render the raw string, not this chip, for an inline id in a table row.                                                                                                                                              |
| `Stepper`                                                                                 | `shared/domain/stepper.tsx`                              | Circles-joined-by-a-line tracker. Complete = filled patina-green + check, current = filled minted-gold + number, upcoming = outline. Drives `INVOICE_LIFECYCLE_STEPS` (Submitted → Buyer review → Tokenized → Funded → Repaid; map a status to its index with `INVOICE_STATUS_STEP_INDEX`) and `WHITELIST_STEPS`, both from `domain-display.ts`. |
