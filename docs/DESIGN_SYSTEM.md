...
(`business/`, `buyer/`, `investor/`, `admin/`, `landing/`) fill in as their screens
are built. Route-specific components stay colocated under the route until a
...
| `InvoiceStatusBadge`, `ProvenanceTierBadge`, `WhitelistStatusBadge`, `OnChainStatusBadge`, `PayoutStatusBadge` | `shared/domain/status-badges.tsx`                        | Wrap `Badge` with the enum → `{label, tone}` lookup from `src/lib/domain-display.ts`. **The only place a status/tier is rendered.** Change a label or colour in `domain-display.ts`, never inline.                                                                                                                                                                                 |
...
- **On-chain**: `confirmed` green, `pending` amber, `failed` red
  (screen 29).
- **Payout**: `received` green, `pending` amber, `failed` red (screen 11).
  Payout statuses use `PayoutStatusBadge` and the shared domain-display
  mapping; the pending payout card separately shows the related invoice
  lifecycle status (for example, `funding`). The action ("Mint" / "Transfer" / "Burn" / "Whitelist")
